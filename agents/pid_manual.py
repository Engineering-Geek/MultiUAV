...
import numpy as np
from mujoco import MjData, MjModel, mj_step, mj_name2id, mjtSensor, mjtObj, viewer, mj_id2name, mj_fullM
...
class ManualDrone:
    ...
    def __init__(self, roll_pid: PID, pitch_pid: PID, yaw_pid: PID,
    ...
    def initialize_motors(self, index):
        ...
        self.max_thrust = self.model.actuator_gear[self.motor_ids[0]][2]
        self.max_torque = self.model.actuator_gear[self.motor_ids[0]][5]
    ...
    def initialize_thrust_matrix(self):
        f = np.abs(self.max_torque / self.max_thrust)
        self.propellers = [
            Propeller(
    ...
    # region Roll, Pitch, Yaw, and Relative Position
    @property
    def roll(self):
    ...
    def thrust(self, global_linear_acceleration: np.ndarray,
        ...
        return np.clip(np.divide(self.A_inv @ np.concatenate([
            rotation_matrix @ (self.drone_mass * global_linear_acceleration),
            rotation_matrix @ (self.I @ global_angular_acceleration)
        ]), self.max_thrust), 0, 1)
